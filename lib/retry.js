...
function afterLaunch(configRetry) {
    ...
    */
    ...
    if (failedTestList.list.failedTest && failedTestList.list.failedTest.length !== 0) {
        ...
        debug('argv.suite:', argv.suite);
        debug('argv.retrySuite:', argv.retrySuite);
        var suite = argv.suite ? argv.suite : argv.retrySuite;
    
        if (!argv.retrySuite && suite) {
            debug('suite defined:', suite);
            protractorCommand.push('--retrySuite', suite);
        }
        ...
        if (retryCount <= retry) {
            process.env.RETRYATTEMPT = retryCount;
...
function resultsFilePath(retry) {
    ...
    var suite = argv.suite ? argv.suite : argv.retrySuite;
    debug('resultsFilePath suite:', suite);

    var resultsDir = path.resolve(process.cwd(), 'protractorFailuresReport', suite ? suite : '');
